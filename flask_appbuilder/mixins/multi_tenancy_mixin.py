...
Row-level security and tenant isolation for SQLAlchemy models in Flask-AppBuilder.
...
def _resolve_session(session: Session | None = None) -> Session:
	"""
	Return *session* if provided; otherwise pull from Flask-AppBuilder's db
...
class MultiTenancyMixin:
	"""
	Row-level security and tenant isolation mixin for Flask-AppBuilder models.
	...
	* Cross-tenant queries only possible via ``CrossTenantQuery`` context
	...
	def get_tenant_query(cls, query: Any = None) -> Any:
		...
		Kept for compatibility with Flask-AppBuilder's ``SQLAInterface`` layer.
...
# ---------------------------------------------------------------------------
# Flask-AppBuilder SQLAInterface drop-in with automatic tenant scoping
# ---------------------------------------------------------------------------

try:
	from flask_appbuilder.models.sqla.interface import SQLAInterface as _SQLAInterface
	...
	log.debug(
		"flask_appbuilder.models.sqla.interface not importable — "
