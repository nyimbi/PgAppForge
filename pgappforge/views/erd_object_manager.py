...
from pgappforge.views.erd_schema_manager import (
	_qi, _safe_type, _validate_pred_expr, _SAFE_VALUE_RE
)
...
_IDENTIFIER_PARAMS = frozenset({
	"name", "schema", "table", "source_table", "table_a", "table_b",
	"policy_name", "owner_col", "tenant_col", "group_col", "value_col",
	"time_col", "order_col", "fk_col", "b_col", "b_alias", "sensitive_col",
	"expiry_col", "admin_role", "privileged_role", "view_name",
})
_INT_PARAMS = frozenset({"lookback_days", "limit_rows", "max_rows"})
_CHANNEL_PARAMS = frozenset({"channel"})
_ALLOWED_POLICY_CMDS = frozenset({"ALL", "SELECT", "INSERT", "UPDATE", "DELETE"})
...
def _validate_view_body(query: str) -> str:
	import re as _re3
	q = query.strip().rstrip(";")
	if not _re3.match(r"^\s*(WITH\b|SELECT\b)", q, _re3.IGNORECASE):
		raise ValueError("VIEW body must start with SELECT or WITH")
	if ";" in q:
		raise ValueError("VIEW body must be a single statement")
	return q
...
class DatabaseObjectManager:
	...
	def apply_object_template(self, template_key: str, **params) -> dict:
		...
		for k, raw in ctx.items():
			v = str(raw)
			if k in _IDENTIFIER_PARAMS:
				validated[k] = _qi(v).strip('"')
			elif k in _INT_PARAMS:
				validated[k] = str(int(v))
			elif k in _CHANNEL_PARAMS:
				import re as _re2
				if not _re2.match(r"^[A-Za-z0-9_-]{1,64}$", v):
					raise ValueError(f"parameter {k!r} is unsafe: {v!r}")
				validated[k] = v
			else:
				if not _SAFE_VALUE_RE.match(v):
					raise ValueError(f"template parameter {k!r} is unsafe: {v!r}")
				validated[k] = v
		sql = tmpl["sql"].format(**validated)
		stmts = [s.strip() for s in sql.split(";") if s.strip()]
		if not stmts:
			return {"applied": 0, "sql": [], "errors": []}
		try:
	...
	def create_view(self, name: str, query: str, schema: str = "public",
		...
		query = _validate_view_body(query)
	...
	def create_policy(self, table: str, name: str, using_expr: str,
		...
		if command.upper() not in _ALLOWED_POLICY_CMDS:
			raise ValueError(f"command must be one of {_ALLOWED_POLICY_CMDS}")
		using_expr = _validate_pred_expr(using_expr)
		if check_expr:
			check_expr = _validate_pred_expr(check_expr)
		stmts = [
