...
class WebSocketManager:
    ...
    async def _authenticate_websocket(self, auth_data: Dict[str, Any]) -> Optional[int]:
        """Authenticate WebSocket connection using PgAppForge security"""
        try:
            ...
            if not hasattr(self.collaboration_engine, "app_builder"):
                logger.error(
                    "No PgAppForge instance available for authentication"
                )
                return None
    ...
    def _get_user_from_connection(
        ...
        """Get PgAppForge user object from connection"""
        try:
            if not hasattr(self.collaboration_engine, "app_builder"):
                logger.error("No PgAppForge instance available for user lookup")
                return None
    ...
    def _validate_session_token(self, user: Any, token: str) -> bool:
        ...
        try:
            # For PgAppForge session-based auth
            ...
            # against PgAppForge's session management
    ...
    async def _validate_message_permissions(
        ...
        try:
            ...
            # Get user from PgAppForge
            if not hasattr(self.collaboration_engine, "app_builder"):
                logger.warning(
                    "No PgAppForge instance available for permission check"
                )
    ...
    async def _validate_message_token(self, connection_id: str) -> bool:
        ...
        try:
            ...
            if not hasattr(self.collaboration_engine, "app_builder"):
                logger.error(
                    "No PgAppForge instance available for token validation"
                )
                return False
