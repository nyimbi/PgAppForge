...
class DataPreviewProfilerWidget(BS3TextFieldWidget):
	...
	def render_field(self, field, **kwargs):
		...
		container_class = "data-profiler-widget"
		if has_errors:
		...
		return Markup(
			...
			<div class="{container_class}" id="{field.id}-container"
				...
				<div class="source-selection mb-3">
					<label for="{field.id}-source" class="form-label">Data Source</label>
				...
				<div class="loading-overlay" style="display:none;" role="alert" aria-busy="true">
					...
					<span class="visually-hidden sr-only">Processing data...</span>
				</div>

				<!-- Error Messages (JS-driven, scoped to this instance) -->
			...
			<script>
				(function() {{
					...
					function initProfiler() {{
						const profiler = new DataProfiler(fieldId, {{
							sourceType: {json.dumps(self.source_type)},
						...
						function showError(error) {{
							// Scope to this field's container — no cross-instance leakage
							var alertEl = document.getElementById(fieldId + '-js-error');
							if (alertEl) {{
						...
						function updateVisualizations(data) {{
							if (data && data.visualizations) {{
								Object.entries(data.visualizations).forEach(([type, config]) => {{
									Plotly.newPlot(fieldId + '-' + type, config);
								}});
							}}
						}}

						// Initialize if data exists
						var existingInput = document.getElementById(fieldId);
						if (existingInput && existingInput.value) {{
							profiler.loadData(JSON.parse(existingInput.value));
						}}

						// Initialize DataTables inside shown.bs.tab to handle hidden-table case
						var tableViewLink = document.querySelector('a[href="#' + fieldId + '-table-view"]');
						if (tableViewLink) {{
							tableViewLink.addEventListener('shown.bs.tab', function() {{
								if (profiler.initDataTable) profiler.initDataTable();
							}});
						}}
					...
					document.addEventListener('shown.bs.tab', function(e) {{
						// Re-trigger if our container becomes visible
						var container = document.getElementById(containerId);
						if (container && container.contains(e.target)) {{ initProfiler(); }}
					}});
					document.addEventListener('shown.bs.modal', initProfiler);
				}})();
			</script>
		"""
		)
	...
	def _render_export_buttons(self, field_id):
		...
		for fmt in self.export_formats:
			# fmt is from self.export_formats (developer-supplied list); escape for HTML safety
			fmt_safe = escape(fmt)
			buttons.append(
				f"""
				<button type="button" class="btn btn-outline-primary"
						data-format="{fmt_safe}"
						aria-label="Export as {fmt_safe.upper()}">
					Export {fmt_safe.upper()}
				</button>
			"""
			)
		return "\n".join(buttons)
