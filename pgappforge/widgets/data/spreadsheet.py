...
class SpreadsheetWidget(BS3TextFieldWidget):
	...
	data_template = (
		'<div class="spreadsheet-wrapper %(wrapper_class)s" style="width:100%%;">'
		...
		"</div>"
		"</div>"
		'<div id="%(field_id)s_hot" class="hot-container" role="grid" aria-label="%(aria_label)s"></div>'
	...
	def __call__(self, field, **kwargs):
		...
		# aria attributes for hidden input
		aria_parts = []
		if has_errors:
			aria_parts.append('aria-invalid="true"')
			aria_parts.append(f'aria-describedby="{field.id}_error"')
		elif description:
			aria_parts.append(f'aria-describedby="{field.id}_help"')
		aria_attrs = " ".join(aria_parts)
	...
	def _get_widget_scripts(self, field):
		...
		# All developer-supplied strings injected into JS via json.dumps() to prevent XSS
		return """
		<script>
			(function() {
				var fieldId = %(field_id_js)s;
				var container = document.getElementById(fieldId + '_hot');
				...
				hot.addHook('afterChange', function() {
					...
					document.getElementById(fieldId).value = JSON.stringify(value);
				});
				...
				function resizeHot() { hot.render(); }
				document.addEventListener('shown.bs.tab', resizeHot);
				document.addEventListener('shown.bs.modal', resizeHot);
