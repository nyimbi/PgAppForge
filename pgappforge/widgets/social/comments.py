...
class CommentAndLikeWidget(BS3TextFieldWidget):
	...
	def render_field(self, field, **kwargs):
		...
		return Markup(
			...
			<div class="comment-widget" id="{escape(field.id)}-container">
				<!-- Comment Form -->
			...
			<script>
				$(document).ready(function() {{
					const comments = new CommentWidget({field_id_js}, {{
						enableReplies: {json.dumps(self.enable_replies)},
						...
						maxLength: {json.dumps(self.max_comment_length)},
						allowedTypes: {_js_json(self.allowed_file_types)},
						sortOptions: {_js_json(self.sort_options)},
						maxNestLevel: {json.dumps(self.max_nest_level)},
						maxAttachments: {json.dumps(self.max_attachments)},
						sizeLimit: {json.dumps(self.attachment_size_limit)},
						mentionMinChars: {json.dumps(self.mention_min_chars)},
	...
	def _render_attachment_upload(self, field_id):
		...
		accepted = escape(",".join(self.allowed_file_types))
		size_mb = self.attachment_size_limit / 1024 / 1024
		types_display = escape(", ".join(self.allowed_file_types))
		return Markup(f"""
			<div class="attachment-upload" role="region" aria-label="File attachments">
				<input type="file" id="{safe_id}-upload" multiple
				       accept="{accepted}"
				       aria-describedby="{safe_id}-upload-help">
				<small id="{safe_id}-upload-help" class="form-text text-muted">
					Allowed files: {types_display}.
					Max size: {size_mb:.1f}MB
				</small>
	...
	def _render_sort_options(self):
		...
		options = "".join(
			f'<option value="{escape(opt)}">{escape(opt.title())}</option>'
			for opt in self.sort_options
		)
		return Markup(
			f'<select class="form-control sort-select" aria-label="Sort comments">'
			f"{options}</select>"
		)
