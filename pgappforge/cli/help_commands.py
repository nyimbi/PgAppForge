"""CLI help and training commands for PgAppForge.
...
from pgappforge.help.views import TOPICS, TRAINING_MODULES
...
def help_topics() -> None:
	...
	_header("PgAppForge Help Topics")
	for slug, topic in TOPICS.items():
		click.echo(
...
def training_group(ctx: click.Context) -> None:
	"""Interactive training modules for PgAppForge.
...
def training_list() -> None:
	...
	_header("PgAppForge Training Modules")
	for mod in TRAINING_MODULES:
		steps = len(mod["steps"])
		click.echo(
