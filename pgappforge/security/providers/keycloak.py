...
class KeycloakAuthProvider:
	...
	def sync_to_fab(self, user: AuthUser, session: Any) -> Any:
		"""Upsert a Keycloak user into FAB's User table for RBAC compatibility."""
		try:
			from flask import current_app
			...
			if fab_user is None:
				...
				fab_user = sm.add_user(
					...
					password="",  # External auth — no local password
				)
			else:
				role_objs = [sm.find_role(r) or sm.add_role(r) for r in user.roles]
			...
			log.warning("KeycloakAuthProvider.sync_to_fab failed: %s", exc)
			return None
