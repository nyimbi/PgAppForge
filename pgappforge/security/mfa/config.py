...
# TOTP Configuration
# -----------------
PGAF_MFA_TOTP_ISSUER = "PgForge"  # Name shown in authenticator apps
PGAF_MFA_TOTP_WINDOW = 1  # Time window tolerance (±30 seconds per window)

# SMS Configuration (Twilio)
# -------------------------
PGAF_MFA_SMS_PROVIDER = "twilio"  # Options: "twilio", "aws_sns"
PGAF_TWILIO_ACCOUNT_SID = "your_twilio_account_sid"
PGAF_TWILIO_AUTH_TOKEN = "your_twilio_auth_token" 
PGAF_TWILIO_FROM_NUMBER = "+1234567890"  # Your Twilio phone number
...
# -------------------------------
PGAF_MFA_EMAIL_PROVIDER = "flask_mail"  # Options: "flask_mail", "sendgrid", "ses"
...
# PGAF_SENDGRID_API_KEY = "your_sendgrid_api_key"
...
# MFA Code Settings
# ---------------
PGAF_MFA_SMS_CODE_EXPIRES = 300  # SMS code expiry in seconds (5 minutes)
PGAF_MFA_EMAIL_CODE_EXPIRES = 600  # Email code expiry in seconds (10 minutes)
PGAF_MFA_MAX_ATTEMPTS = 5  # Maximum verification attempts before lockout
PGAF_MFA_LOCKOUT_DURATION = 30  # Lockout duration in minutes

# MFA Policy Settings
# -----------------
PGAF_MFA_REQUIRE_BACKUP_CODES = True  # Require backup codes for MFA setup
PGAF_MFA_BACKUP_CODE_COUNT = 10  # Number of backup codes to generate
PGAF_MFA_GRACE_PERIOD_DAYS = 7  # Grace period before MFA enforcement
PGAF_MFA_REMINDER_DAYS = 3  # Days before grace period ends to send reminders

# Security Settings
# ---------------
PGAF_MFA_RATE_LIMIT_ENABLED = True  # Enable rate limiting for MFA endpoints
PGAF_MFA_RATE_LIMIT_WINDOW = 300  # Rate limit window in seconds (5 minutes)
PGAF_MFA_RATE_LIMIT_MAX_ATTEMPTS = 10  # Max attempts per window per IP

# Session Settings
# --------------
PGAF_MFA_SESSION_TIMEOUT = 900  # MFA session timeout in seconds (15 minutes)
PGAF_MFA_REMEMBER_DEVICE = False  # Allow "remember this device" option
PGAF_MFA_REMEMBER_DEVICE_DAYS = 30  # Days to remember device
...
def get_mfa_config_template():
    ...
    return {
        # Core MFA Settings
        'PGAF_MFA_ENABLED': False,  # Set to True to enable MFA
        'PGAF_MFA_TOTP_ISSUER': 'PgForge',
...
def validate_mfa_config(config_dict):
    ...
    if not config_dict.get('PGAF_MFA_ENABLED'):
    ...
    if not config_dict.get('PGAF_MFA_TOTP_ISSUER'):
        errors.append("PGAF_MFA_TOTP_ISSUER should be set to your application name")
    
    # Check SMS provider configuration
    sms_provider = config_dict.get('PGAF_MFA_SMS_PROVIDER', 'twilio')
    if sms_provider == 'twilio':
        twilio_settings = ['PGAF_TWILIO_ACCOUNT_SID', 'PGAF_TWILIO_AUTH_TOKEN', 'PGAF_TWILIO_FROM_NUMBER']
    ...
    email_provider = config_dict.get('PGAF_MFA_EMAIL_PROVIDER', 'flask_mail')
    if email_provider == 'flask_mail':
        ...
        if not config_dict.get('PGAF_SENDGRID_API_KEY'):
            errors.append("SendGrid requires PGAF_SENDGRID_API_KEY")
