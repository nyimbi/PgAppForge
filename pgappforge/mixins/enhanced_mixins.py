"""
Enhanced Mixins for PgAppForge

This module provides enhanced mixins that extend PgAppForge's base functionality
with advanced features from the appgen project. These mixins are designed to work
seamlessly with PgAppForge's existing architecture while providing additional
...
These mixins maintain backward compatibility with PgAppForge's existing
...
class EnhancedSoftDeleteMixin(AuditMixin):
    ...
    Extends PgAppForge's AuditMixin to add sophisticated soft delete
    ...
    - Integration with PgAppForge's security model
    """
    ...
    def _get_current_user_id():
        """Get current user ID from PgAppForge context."""
        try:
...
class CacheableMixin:
    """
    Caching mixin for PgAppForge models.
    
    Provides instance-level caching with automatic invalidation
    on model updates. Integrates with PgAppForge's caching
    infrastructure.
    """
...
class ImportExportMixin:
    """
    Data import/export mixin for PgAppForge models.
...
def setup_enhanced_mixins(app):
    """
    Set up enhanced mixins with PgAppForge including performance optimizations.
