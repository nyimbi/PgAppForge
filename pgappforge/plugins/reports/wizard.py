...
# Per-tab isolation: each wizard instance gets its own session key based on
# a UUID passed in the URL query string (?wiz=<uuid>). If absent, we use a
# shared key — this degrades to the old behaviour (one wizard per browser).
_SESS_PREFIX = "reportforge_wizard_"


def _wiz_id() -> str:
    """Return the wizard instance id from ?wiz= query param, or a default."""
    return request.args.get("wiz") or "default"


def _sess_key() -> str:
    return _SESS_PREFIX + _wiz_id()
...
def _step3_html(state: dict) -> str:
    ...
    cn   = _he(b.get("company_name", ""))
    lu   = _he(b.get("logo_url", ""))
    wm   = _he(b.get("watermark_text", ""))
    pc   = _he(b.get("primary_color",   "#003366"))
    sc   = _he(b.get("secondary_color", "#666666"))
    cfh  = _he(b.get("custom_footer_html", ""))
    chh  = _he(b.get("custom_header_html", ""))
    return f"""
<div class="card p-4">
  ...
  <form method="POST" action="/reportforge/wizard/step/3">
    <div class="row g-3">
      <div class="col-6">
        ...
        <input name="company_name" class="form-control" value="{cn}">
      </div>
      <div class="col-6">
        <label class="form-label">Logo URL <span class="text-muted">(optional)</span></label>
        <input name="logo_url" class="form-control" placeholder="https://…/logo.png" value="{lu}">
      </div>
      <div class="col-4">
        <label class="form-label">Primary Colour</label>
        <input type="color" name="primary_color" class="form-control form-control-color" value="{pc}">
      </div>
      <div class="col-4">
        <label class="form-label">Secondary Colour</label>
        <input type="color" name="secondary_color" class="form-control form-control-color" value="{sc}">
      </div>
      <div class="col-4">
        <label class="form-label">Watermark Text <span class="text-muted">(optional)</span></label>
        <input name="watermark_text" class="form-control" placeholder="CONFIDENTIAL" value="{wm}">
      </div>
      <div class="col-6">
        <label class="form-label">Custom Footer</label>
        <textarea name="custom_footer_html" class="form-control" rows="2"
          placeholder="Page {{page}} · Generated by ReportForge">{cfh}</textarea>
      </div>
      <div class="col-6">
        ...
        <textarea name="custom_header_html" class="form-control" rows="2"
          placeholder="Optional HTML header content">{chh}</textarea>
      </div>
    </div>
...
def _step5_html(state: dict) -> str:
    default_name = _he(state.get("name", ""))
    desc = _he(state.get("description", ""))
    return f"""
<div class="card p-4">
  ...
  <form method="POST" action="/reportforge/wizard/step/5">
    ...
    <div class="mb-3">
      ...
      <textarea name="description" class="form-control" rows="2"
                placeholder="What does this report show?">{desc}</textarea>
    </div>
    <div class="row g-3">
      <div class="col-4">
...
def _validate_template_sql(sql: str) -> str:
    """
    Run EXPLAIN on the sample SQL (with dummy parameter values) to detect
    missing tables/columns early. Returns a warning string, or "" if OK.
    """
    try:
        ...
        if not appbuilder:
            return ""
        # Replace :param placeholders with NULL so EXPLAIN doesn't fail on missing binds
        explain_sql = re.sub(r":\w+", "NULL", sql)
        with appbuilder.session.bind.connect() as conn:
            conn.execute(sa.text(f"EXPLAIN {explain_sql}"))
        return ""
    except Exception as exc:
        msg = str(exc)
        # Extract the first meaningful line for brevity
...
class ReportWizardView(BaseView):
	...
	def step1_post(self):
		...
		sql_warning = ""
		if tmpl:
			sample_sql = tmpl.get("sample_sql", "")
			state["sql"] = sample_sql
			state["branding"] = {
			...
			# Validate sample SQL — detect missing tables early
			if sample_sql:
				sql_warning = _validate_template_sql(sample_sql)

		_save_state(state)
		body = _step2_html(state)
		if sql_warning:
			# Prepend a non-fatal advisory banner
			banner = (
				f'<div class="alert alert-warning"><i class="fas fa-exclamation-triangle me-2"></i>'
				f'<strong>Template SQL note:</strong> {_he(sql_warning)} '
				f'Edit the SQL in this step before proceeding.</div>'
			)
			body = banner + body
		return _render(2, body, _step2_js())
...
def _generate_preview(state: dict) -> tuple[str, str | None]:
	...
	try:
		...
		for i, row in enumerate(rows):
			...
			for col in columns:
				val = row.get(col, "")
				safe_val = _he(val) if val is not None else ""
