...
class PluginManager:
	"""
	Enhanced plugin manager for PgForge.
	...
	# ─── Config-based loading + hook wiring ─────────────────────────────────────

	def init_from_app_config(self, app, appbuilder) -> None:
		"""Load plugins specified in Flask app config.

		Reads these config keys:
		  PGAPPFORGE_PLUGINS          list[str] — plugin names to load
		  PGAPPFORGE_AUTOLOAD_PLUGINS bool      — scan entry_points for plugins
		  PGAPPFORGE_PLUGIN_CONFIGS   dict      — per-plugin config overrides

		Example::

		    app.config['PGAPPFORGE_PLUGINS'] = ['analytics', 'workflow']
		    app.config['PGAPPFORGE_PLUGIN_CONFIGS'] = {
		        'analytics': {'default_chart_type': 'bar'},
		    }
		"""
		plugin_names = app.config.get("PGAPPFORGE_PLUGINS", [])
		autoload = app.config.get("PGAPPFORGE_AUTOLOAD_PLUGINS", False)
		per_plugin_cfg = app.config.get("PGAPPFORGE_PLUGIN_CONFIGS", {})
		...
		for name in plugin_names:
			cfg = per_plugin_cfg.get(name, {})
			try:
				self.load_plugin(name, config=cfg)
				logger.info("Loaded plugin from config: %s", name)
			except Exception as exc:
				logger.error("Failed to load plugin %s: %s", name, exc)

	def wire_hooks(self, hooks) -> None:
		"""Connect HookRegistry signals to all active plugins' hook methods.

		Call after all plugins are loaded::

		    plugin_manager.init_from_app_config(app, appbuilder)
		    plugin_manager.wire_hooks(appbuilder.hooks)
		"""
		active = [p for p in self.registry._plugins.values()
		          if p.status.name == "ACTIVE"]

		for plugin in active:
			if hasattr(plugin, "on_app_ready"):
				hooks.on_app_ready.connect(lambda app, p=plugin: p.on_app_ready(app))
			if hasattr(plugin, "on_user_login"):
				hooks.on_user_login.connect(lambda user, p=plugin: p.on_user_login(user))
			if hasattr(plugin, "on_user_logout"):
				hooks.on_user_logout.connect(lambda user, p=plugin: p.on_user_logout(user))
			if hasattr(plugin, "on_record_save"):
				hooks.on_record_save.connect(
					lambda cls, rec, is_new, p=plugin: p.on_record_save(cls, rec, is_new)
				)
			if hasattr(plugin, "on_record_delete"):
				hooks.on_record_delete.connect(
					lambda cls, rec, p=plugin: p.on_record_delete(cls, rec)
				)
			if hasattr(plugin, "on_permission_denied"):
				hooks.on_permission_denied.connect(
					lambda user, perm, vm, p=plugin: p.on_permission_denied(user, perm, vm)
				)
		logger.info("Wired hooks for %d active plugins", len(active))

	@staticmethod
	def _discover_entry_points() -> dict:
		"""Scan installed packages for pgappforge_plugins entry_points."""
		import importlib.metadata as meta
		discovered = {}
		try:
			eps = meta.entry_points(group="pgappforge_plugins")
			for ep in eps:
				try:
					cls = ep.load()
					discovered[ep.name] = cls
					logger.info("Discovered plugin via entry_point: %s", ep.name)
				except Exception as exc:
					logger.warning("Could not load plugin entry_point %s: %s", ep.name, exc)
		except Exception as exc:
